...
export def --env --wrapped main [command?: string, --help, ...rest: string] {
  ...
  if ($command == null) {
    ^"/etc/profiles/per-user/kyohei/bin/starship"
  } else if ($command == "activate") {
    $env.MISE_SHELL = "nu"
  } else if ($command in $commands) {
    ^"/etc/profiles/per-user/kyohei/bin/starship" $command ...$rest
    | parse vars
    | update-env
  } else {
    ^"/etc/profiles/per-user/kyohei/bin/starship" $command ...$rest
...
def --env mise_hook [] {
  ^"/etc/profiles/per-user/kyohei/bin/starship" hook-env -s nu
    | parse vars
    | update-env
}
